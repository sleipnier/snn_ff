...
# Sweep BP-SNN across time steps T for 4 neuron types.
# Results are written under: ./result/timestep/<model>/T<T>/

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TRAIN_SCRIPT="${TRAIN_SCRIPT:-$SCRIPT_DIR/mnist_bp_train_v2.py}"
...
run_one() {
  ...
  # Best parameters from existing BP best runs (model-specific if needed).
  local hidden_dim=500
  local num_layers=2
  local lr=0.001
  local weight_decay=0.0
  local input_gain=1.0
  local v_threshold=0.4
  local v_reset=0.0
  local tau=2.0
  ...
  echo "=============================================================="
  echo "[BP] model=$model | T=$T | device=$gpu | out_dir=$out_dir"
  python "$TRAIN_SCRIPT" \
    -device "$gpu" \
    -data-dir "$DATA_DIR" \
    -out-dir "$out_dir" \
...
echo
echo "BP timestep sweep finished."
