...
def parse_args():
    p = argparse.ArgumentParser(description="Refined NMNIST FF-SNN comparison with 4 neurons + metrics")
...
if __name__ == "__main__":
    ...
    print(args)
    print(f"Sample frame shape: {tuple(sample_frame.shape)}, label={sample_label}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        eval_train_start = time.time()
        train_stats = eval_loader(net, train_loader, device=device,
                                  label_scale=args.label_scale,
                                  num_classes=args.num_classes,
                                  collect_stats=False)
        eval_train_elapsed = time.time() - eval_train_start
        train_preds, train_targets = train_stats.preds, torch.cat([y for _, y in train_loader]) if False else (train_stats.preds, None)
        # targets from loader are not retained above; compute metrics by a second cheap pass using eval outputs only is not possible.
        # So compute train metrics in a dedicated loop below.
