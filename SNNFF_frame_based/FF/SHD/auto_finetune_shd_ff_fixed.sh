...
ROOT_DIR="${ROOT_DIR:-/home/yhxu/spikingjelly/spikingjelly/spikingjelly/SNNFF/FF/SHD/result/fintune/shd_ff}"
mkdir -p "$ROOT_DIR"
...
TRAIN_SCRIPT="${TRAIN_SCRIPT:-/home/yhxu/spikingjelly/spikingjelly/spikingjelly/SNNFF/FF/SHD/shd_ff_train_v1.py}"
...
def sample_cfg(model):
    common = {
        ...
        "v_reset": 0.0,
        "alpha": random.choice([4.0, 6.0, 8.0]),
        "label_scale": random.choice([0.8, 1.0, 1.2, 1.5]),
    }
...
def build_cmd(cfg, out_dir, model, gpu):
    ...
    if "alpha" in cfg: cmd += ["--alpha", str(cfg["alpha"])]
    if "label_scale" in cfg: cmd += ["--label-scale", str(cfg["label_scale"])]
