...
parser = argparse.ArgumentParser(description="SHD FF-SNN with four neuron types")
...
def save_model_artifact(path: str, net_: nn.Module, extra_metadata: Optional[Dict[str, object]]=None):
    payload = {"model_state_dict": net_.state_dict(), "args": vars(args), "dims": dims, "model_class": net_.__class__.__name__, "input_size": INPUT_SIZE, "num_classes": NUM_CLASSES}
    if extra_metadata is not None:
        payload["metadata"] = extra_metadata
    torch.save(payload, path)

def overlay_y_on_x_temporal(x: torch.Tensor, y: Union[torch.Tensor, int], num_classes=NUM_CLASSES, label_scale=1.0):
    x = x.clone()
    B = x.size(0)
    if isinstance(y, int):
    ...
    x[:, :, :num_classes] = 0.0
    vmax = x.abs().amax(dim=(1,2)) + 1e-6
    b = torch.arange(B, device=x.device)
    x[b, :, y.clamp(0, num_classes-1)] = vmax[b].unsqueeze(1) * float(label_scale)
    return x
...
class FFSpikingNet(nn.Module):
    ...
    def layer_goodnesses(self, x_btf: torch.Tensor, y_overlay: Union[torch.Tensor, int], activity_summary=None):
        x = overlay_y_on_x_temporal(x_btf, y_overlay, label_scale=args.label_scale)
...
def make_examples(model: FFSpikingNet, x_btf: torch.Tensor, y_true: torch.Tensor, epsilon: float=1e-12):
    ...
    x_pos = overlay_y_on_x_temporal(x_btf, y_true, label_scale=args.label_scale)
    x_neg = overlay_y_on_x_temporal(x_btf, y_hard, label_scale=args.label_scale)
...
artifact_prefix = os.path.join(args.out_dir, f"SHD_{args.model}_FF_{run_id}")
